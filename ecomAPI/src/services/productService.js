    ...
    try {
      ...
      for (let i = 0; i < res.rows.length; i++) {
        ...
        for (let j = 0; j < res.rows[i].productDetail.length; j++) {
          ...
          for (
            let k = 0;
            k < res.rows[i].productDetail[j].productImage.length;
            k++
          ) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      for (let i = 0; i < res.rows.length; i++) {
        ...
        if (
          !res.rows[i].productDetail ||
          res.rows[i].productDetail.length === 0
        ) {
          res.rows[i].price = null;
          continue;
        }
        ...
        for (let j = 0; j < res.rows[i].productDetail.length; j++) {
          ...
          res.rows[i].price = res.rows[i].productDetail[0]
          ...
          for (
            ...
            res.rows[i].productDetail[j].productImage[k].image = toBinaryImage(
              res.rows[i].productDetail[j].productImage[k].image
            );
          }
        }
        ...
        if (productdetail.rows && productdetail.rows.length > 0) {
          for (let i = 0; i < productdetail.rows.length; i++) {
            ...
            if (
              ...
              for (
                let j = 0;
                j < productdetail.rows[i].productImageData.length;
                j++
              ) {
                productdetail.rows[i].productImageData[j].image = toBinaryImage(
      ...
      if (!data.id || !data.limit || !data.offset) {
        ...
        if (productImage.rows && productImage.rows.length > 0) {
          productImage.rows.map(
            (item) => (item.image = toBinaryImage(item.image))
          );
        }
    ...
    try {
      if (!id) {
        ...
        if (productdetailImage) {
          productdetailImage.image = toBinaryImage(productdetailImage.image);
        }
        resolve({
          errCode: 0,
        ...
        if (productArr && productArr.length > 0) {
          for (let g = 0; g < productArr.length; g++) {
            ...
            for (let j = 0; j < productArr[g].productDetail.length; j++) {
              ...
              for (
                let k = 0;
                k < productArr[g].productDetail[j].productImage.length;
                k++
              ) {
